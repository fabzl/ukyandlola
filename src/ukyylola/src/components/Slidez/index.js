...
class Slideshow extends Component {
	...
	handleKeyboard(e) {
		return e.keyCode === 37 ? this.decreaseCount() : e.keyCode === 39 ? this.increaseCount() : null;
	}
	...
	render() {
		...
		if (!this.props.children) {
			slideShowSlides = slides.map((slide, i) => {
				return (
					<li
						className={`slide ${effect} ${this.state.currentSlide === i ? 'showing-' + slideEffect : ''}`}
						key={i}
		...
		return (
			...
			>
				<div className="slideshow-container">
					...
					{showArrows && <Arrows decreaseCount={this.decreaseCount} increaseCount={this.increaseCount} />}
